...
export function requireActiveTenant() {
  if (!activeTenant?.negocio?.id) {
    throw new Error("No hay un negocio activo en esta sesión.");
  }
...
export async function resolveTenantForSession(session) {
  ...
  // Por ahora el sistema trabaja con un solo establecimiento activo por cuenta.
  // La BD puede soportar varios en el futuro, pero no necesitamos esa función ahora.
  ...
  const [
    ...
    { data: suscripcion, error: suscripcionError },
    { data: configuracion, error: configuracionError },
  ] = await Promise.all([
    supabase
      .from("negocios")
    ...
    supabase
      .from("suscripciones")
      .select("negocio_id, plan_code, estado, starts_at, ends_at")
    ...
    supabase
      ...
      .select(
        "negocio_id, nombre_comercial, logo_url, nit, direccion, telefono, timezone, moneda, reglas_pedidos"
      )
      .eq("negocio_id", membership.negocio_id)
      .maybeSingle(),
  ]);
  ...
  if (!negocio) {
    throw new Error("No se encontró el establecimiento de esta cuenta.");
  }
  ...
  if (
    ...
    new Date(suscripcion.starts_at).getTime() > now
  ) {
    throw new Error(
  ...
  if (
    suscripcion.ends_at &&
    new Date(suscripcion.ends_at).getTime() <= now
  ) {
    throw new Error(
